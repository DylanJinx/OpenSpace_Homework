...
contract Dylan_NFTMarket_TwoSign_Test is Test {
    ...
    function test_SuccessBuyNFT_WLSign_ERC20Permit() public {
        ...
        vm.prank(nftBuyer);
        marketContract.buy(orderHashId_, _approveData, wlData.signature);
    ...
    function test_FailBuyNFT_WLSign_ERC20Permit() public {
        // This NFT requires whitelist purchase
        uint256 tokenId_= nftSellerListNFT(true);
        bytes32 orderHashId_ = marketContract.isListed(address(nftContract), tokenId_);
        // give the nftBuyer some tokens
        ...
        vm.prank(nftBuyer);
        marketContract.buy(orderHashId_, _approveData);
    } 

    // need handling fee
    function test_SuccessBuyNFT_WLSign_ERC20Permit_fee() public {
        // This NFT requires whitelist purchase
        uint256 tokenId_= nftSellerListNFT(true);
        bytes32 orderHashId_ = marketContract.isListed(address(nftContract), tokenId_);
        // give the nftBuyer some tokens
