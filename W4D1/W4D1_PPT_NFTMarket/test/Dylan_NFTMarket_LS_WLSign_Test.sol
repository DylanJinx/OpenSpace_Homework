...
contract Dylan_NFTMarket_LS_WLSign_Test is Test {
    ...
    // --------------------------------------------------------------tests--------------------------------------------------------------
    // use token buy NFT
    function test_Success_Token_Three_WLS_LS() public {
        ...
        vm.prank(nftBuyer);
        marketContract.buy(wlData.signature, listSignature);
    ...
    // use ETH buy NFT
    function test_Success_ETH_Three_WLS_LS() public {
        uint256 beforeBuyerETH =  nftBuyer.balance;
        uint256 beforeSellerETH =  nftSeller.balance;
        console.log("BuyerETH:", beforeBuyerETH);
        console.log("SellerETH:", beforeSellerETH);
        // This NFT requires whitelist purchase
        // uint256 tokenId_= nftSellerListNFT(true);
        uint256 tokenId_ = mintNFT();
        ...
        vm.prank(nftBuyer);
        marketContract.buy{value: nftPrice}(wlData.signature, listSignature);

        uint256 afterBuyerETH =  nftBuyer.balance;
        uint256 afterSellerETH =  nftSeller.balance;
        console.log("BuyerETH:", afterBuyerETH);
        console.log("SellerETH:", afterSellerETH);

        assertEq(nftContract.ownerOf(tokenId_), nftBuyer, "NFT was not transferred to the buyer");
        assertEq(afterSellerETH - beforeSellerETH, 1e18, "Seller did not receive the tokens");
        assertEq(beforeBuyerETH - afterBuyerETH, 1e18, "Buyer still has tokens");
    } 
