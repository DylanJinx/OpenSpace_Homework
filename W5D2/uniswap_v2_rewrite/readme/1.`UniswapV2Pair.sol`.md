...
由上面可得，  
$x*\Delta y + \Delta x*\Delta y = \Delta x * y => \Delta y = \frac{\Delta x * y}{x + \Delta x}$

### Spot Price (现货价格)
在使用 x 交换成 y 的时候，显示的价格为 $P_0=\frac{y}{x}$，但实际成交价格为 $P_1=\frac{\Delta y}{\Delta x}$，$P_0$ 和 $P_1$ 之间的差值就是所谓的滑点  
$x_y &= (x+\Delta x)_(y-\Delta y) \\ P_1 = \frac{\Delta y}{\Delta x} &= \frac{y}{x+\Delta x}$  
